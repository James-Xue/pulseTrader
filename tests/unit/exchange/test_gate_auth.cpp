...
//   3. buildSignPayload produces the correct canonical format
//   4. signRequest end-to-end produces a valid signature
//   5. unixSeconds returns a plausible timestamp
...
TEST(GateAuthTest, Sha512EmptyString)
{
    ...
    const std::string result = sha512Hex("");
...
TEST(GateAuthTest, Sha512Abc)
{
    ...
    const std::string result = sha512Hex("abc");
    EXPECT_EQ(128u, result.size());
...
TEST(GateAuthTest, Sha512JsonBody)
{
    ...
    const std::string hash1 = sha512Hex(body);
    const std::string hash2 = sha512Hex(body);
...
TEST(GateAuthTest, HmacSha512KnownVector)
{
    ...
    const std::string result = hmacSha512Hex("Jefe", "what do ya want for nothing?");
    EXPECT_EQ(128u, result.size());
...
TEST(GateAuthTest, HmacSha512Deterministic)
{
    ...
    const std::string sig1 = hmacSha512Hex(secret, payload);
    const std::string sig2 = hmacSha512Hex(secret, payload);
    EXPECT_EQ(sig1, sig2);
}
...
TEST(GateAuthTest, UnixSecondsIsPlausible)
{
    ...
    const std::string ts_str = unixSeconds();
...
// ---------------------------------------------------------------------------
// buildSignPayload tests
// ---------------------------------------------------------------------------

TEST(GateAuthTest, BuildSignPayloadFormat)
{
    // Verify the canonical format: METHOD\nPATH\nQUERY\nBODY_HASH\nTIMESTAMP
    const std::string result = buildSignPayload("GET", "/api/v4/spot/accounts", "", "", "1700000000");
    ...
    const std::string empty_body_hash = sha512Hex("");
...
TEST(GateAuthTest, BuildSignPayloadWithQuery)
{
    ...
    const std::string result =
            buildSignPayload("GET", "/api/v4/spot/tickers", "currency_pair=BTC_USDT", "", "1700000000");
...
TEST(GateAuthTest, BuildSignPayloadWithBody)
{
    ...
    const std::string payload1 = buildSignPayload("POST", "/api/v4/spot/orders", "", body1, "1700000000");
    const std::string payload2 = buildSignPayload("POST", "/api/v4/spot/orders", "", body2, "1700000000");

    EXPECT_NE(payload1, payload2); // different bodies → different payloads
}

// ---------------------------------------------------------------------------
// signRequest end-to-end test
// ---------------------------------------------------------------------------
...
TEST(GateAuthTest, SignRequestEndToEnd)
{
    ...
    const std::string sig1 = signRequest(secret, method, path, query, body, timestamp);
    const std::string sig2 = signRequest(secret, method, path, query, body, timestamp);

    EXPECT_EQ(128u, sig1.size());
    EXPECT_EQ(sig1, sig2);

    // Verify it is actually an HMAC of the buildSignPayload
    const std::string payload = buildSignPayload(method, path, query, body, timestamp);
    const std::string expected = hmacSha512Hex(secret, payload);
...
TEST(GateAuthTest, SignRequestDifferentSecretsDifferentSigs)
{
    ...
    const std::string sig1 = signRequest("secret_a", "GET", "/api/v4/spot/accounts", "", "", "1700000000");
    const std::string sig2 = signRequest("secret_b", "GET", "/api/v4/spot/accounts", "", "", "1700000000");
    EXPECT_NE(sig1, sig2);
}
