...
**Milestones M1–M9 achieved** — all 9 layers operational, TOML configuration, SQLite trade recording, EndpointRouter for spot/futures routing, and a full trading engine wiring all layers into a single runnable process.
...
- **Gate.io spot integration** — Native REST (HMAC-SHA512 signed) and WebSocket channels (order book, trades, tickers, K-lines, private orders), incremental order book updates, proxy tunnel support.
...
./run.sh test        # Run all 467 unit tests
```
...
| M10 | Futures market data (mark_price, funding_rate) | 🔲 |
...
- [ ] **Futures support** — Gate.io USDT perpetual contracts (M10–M12: market data, risk/PnL, execution)
